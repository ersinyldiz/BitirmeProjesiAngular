...
@Component({
  ...
  templateUrl: './yardimonaybekleyen.component.html',
  styleUrls: ['./yardimonaybekleyen.component.scss']
})

export class YardimOnayBekleyenComponent implements OnInit {
  ...
  veriGetir() {
    ...
    this.dataservis.GetBasvurularByYardimOnayiBekleyen().subscribe(pagedData => {
      this.rows = pagedData;
  ...
  onayla() {
    ...
    swal({
      title: 'Uyarı!',
      text: 'Bu Yardımlar Onaylansın mı ?',
      type: 'warning',
      ...
      if (result.value) {
        this.dataservis.YardimiOnayla(this.form.value).subscribe(pagedData => {
          if (pagedData.SonucMesaji === '0000') {
            this.toastr.success({
              title: 'Sonuç Mesajı',
              msg: 'Seçmiş Olduğunuz Yardımlar Başarı ile Onaylanmıştır.',
              timeout: 5000,
              theme: 'default'
            });
            this.veriGetir();
  ...
  reddet() {
    ...
    swal({
      title: 'Uyarı!',
      text: 'Bu Yardımlar İptal Edilsin mi ?',
      type: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#d33',
      cancelButtonColor: '#3085d6',
      ...
      if (result.value) {
        this.dataservis.YardimiİptalEt(this.form.value).subscribe(pagedData => {
          if (pagedData.SonucMesaji === '0000') {
            this.toastr.success({
              title: 'Sonuç Mesajı',
              msg: 'Seçmiş Olduğunuz Yardımlar Başarı ile İptal Edilmiştir.',
              timeout: 5000,
              theme: 'default'
            });
            this.veriGetir();

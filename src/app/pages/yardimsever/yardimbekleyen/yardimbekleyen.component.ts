...
@Component({
  selector: 'app-yardimbekleyen',
  templateUrl: './yardimbekleyen.component.html',
  styleUrls: ['./yardimbekleyen.component.scss']
})

export class YardimBekleyenComponent implements OnInit {
  ...
  veriGetir() {
    ...
    this.dataservis.GetBasvurularByYardimBekleyen().subscribe(pagedData => {
      this.rows = pagedData;
  ...
  onayla() {
    ...
    swal({
      title: 'Uyarı!',
      text: 'Bu Yardım Başvurularını Karşılamak İstiyor musunuz ?',
      type: 'warning',
      ...
      if (result.value) {
        this.dataservis.BasvuruyaYardimEt(this.form.value).subscribe(pagedData => {
          if (pagedData.SonucMesaji === '0000') {
            this.toastr.success({
              title: 'Sonuç Mesajı',
              msg: 'Seçmiş Olduğunuz Başvurulara Yardım Yapma İsteğiniz Alınmıştır. Yardımınız için Teşekkür Ederiz. Muhtar Aracılığı ile Yardımınızı Gerçekleştirebilirsiniz',
              timeout: 5000,
              theme: 'default'
            });
            this.veriGetir();
